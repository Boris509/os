...
int pseudos_verification(const char* pseudo_utilisateur, const char* pseudo_destinataire, size_t size_pseudo, const char* non_valid_character) {
    printf("size of pseudo dest :%zu", size_pseudo);
...
// Fonction pour afficher les messages en attente et réinitialiser la mémoire
/*void display_pending_messages(bool signale ) {
   ...
   char *message_start = shared_memory + MESSAGE_OFFSET; // Message area
    if (*used_memory_ptr == 0 && signale == true) {
        printf("No messages in shared memory.\n");
    } else if(*used_memory_ptr > 0){
        memset(message_start, 0, MEMORY_SIZE - MESSAGE_OFFSET); // Clear the message area
...
void display_pending_messages(bool from_interrupt) {
    ...
    if (*used_memory_ptr == 0 && from_interrupt) {
        ...
        while (remaining > 0) {
            ...
            printf("%zu", remaining); // Include the null terminator
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 3) {
        fprintf(stderr, "Usage: %s pseudo_utilisateur pseudo_destinataire [--bot | --manuel]\n", argv[0]);
        return 1;
    }

    // Determine the mode
    if (argc < 4) {
        // Default to bot mode if mode argument is not provided
        bot_mode = true;
    } else if (strcmp(argv[3], "--bot") == 0) {
        bot_mode = true;
    } else if (strcmp(argv[3], "--manuel") == 0) {
        manual_mode = true;
    } else {
        fprintf(stderr, "Error: Unknown mode. Use --bot or --manuel.\n");
        return 1;
    }
